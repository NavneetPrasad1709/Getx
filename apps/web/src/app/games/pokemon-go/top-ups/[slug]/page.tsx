        ...
        <main className="flex-1 container py-16">
          <Card>
            <CardContent className="p-12 text-center">
              <h1 className="font-display text-2xl font-bold mb-2">Top up not found</h1>
              <p className="text-muted-foreground mb-6">
                This listing may have been sold or removed.
              </p>
              <Link href="/games/pokemon-go/top-ups">
                <Button>Browse all top ups</Button>
              </Link>
            </CardContent>
          </Card>
        </main>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container py-8">
        ...
        >
          ...
          <span aria-hidden="true">/</span>
          <Link href="/games/pokemon-go/top-ups" className="hover:text-foreground">
            Top Ups
          </Link>
          <span aria-hidden="true">/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <TopUpStats attrs={listing.attributes} />
        ...
        {related && related.length > 0 && (
          <section className="mt-16">
            <h2 className="font-display text-2xl font-bold mb-6">Similar top ups</h2>
    ...
    <div className="space-y-3">
      <div className="aspect-video rounded-lg overflow-hidden bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center">
        {activeImage ? (
          ...
          <span className="font-display text-6xl font-bold text-primary/40">T</span>
        )}
      </div>
...
function TopUpStats({ attrs }: { attrs: Record<string, unknown> }) {
  const coinAmount = attrString(attrs, 'coinAmount');
  const deliveryMethod = attrString(attrs, 'deliveryMethod');
  const platform = attrString(attrs, 'platform');

  if (!coinAmount && !deliveryMethod && !platform) return null;

  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Top Up Details</CardTitle>
      </CardHeader>
      <CardContent>
        <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {coinAmount && <Stat label="Coin Amount" value={`${coinAmount} coins`} />}
          {deliveryMethod && <Stat label="Delivery Method" value={deliveryMethod} />}
          {platform && <Stat label="Platform" value={platform} />}
        </dl>
      </CardContent>
    </Card>
  );
