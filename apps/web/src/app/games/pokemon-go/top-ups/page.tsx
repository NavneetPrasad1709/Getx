...
import { TopUpsFilters } from '@/components/listings/top-ups-filters';
...
function TopUpsBrowseContent() {
  ...
  const filters: ListingFilters = {
    ...
    coinAmount: searchParams.get('coinAmount') ?? undefined,
  ...
  if (filters.coinAmount) {
    chips.push({ key: 'coinAmount', label: `${filters.coinAmount} coins`, onRemove: () => updateFilters({ coinAmount: undefined }) });
  }
  if (filters.deliveryMethod) {
    chips.push({ key: 'deliveryMethod', label: filters.deliveryMethod, onRemove: () => updateFilters({ deliveryMethod: undefined }) });
  }
  if (filters.platform) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <BrowseHeader
        eyebrow="Pokémon GO · Top-ups"
        title="PokéCoins, delivered."
        subtitle="Bulk-discounted coins direct to your Trainer ID. Most orders ship in under 5 minutes."
        trail={[
          { href: '/', label: 'Home' },
      ...
      <PillFilterBar
        ...
        saveSearchSlot={<SaveSearchButton gameSlug="pokemon-go" tabType="TOP_UPS" />}
      />
      ...
      <main className="flex-1 container pb-20">
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8">
          ...
          <div>
            ...
            {data && data.data.length === 0 && (
              <EmptyState
                title="No top-ups match those filters"
                body="Try widening the range, or request the exact pack you need."
                actions={
                  <>
                    <Button onClick={clearFilters}>Clear filters</Button>
                    <CustomRequestButton gameSlug="pokemon-go" tabType="TOP_UPS" variant="outline" />
                  </>
                }
              />
            )}
            ...
            {data && data.data.length > 0 && (
              <>
                ...
                <div className="mt-12">
                  <CustomRequestCTA gameSlug="pokemon-go" tabType="TOP_UPS" variant="banner" />
                </div>
              </>
            )}
          </div>
        ...
        <FloatingCTA gameSlug="pokemon-go" tabType="TOP_UPS" />
      </main>
...
export default function TopUpsBrowsePage() {
  return (
